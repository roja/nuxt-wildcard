...
export default {
  ...
  async asyncData (context) {
    let {meaningOfLife} = endpoints;

    if( process.server ) {
      meaningOfLife = meaningOfLife.bind(context);
    }

    const theMeaning = await meaningOfLife();

    return   {
      theMeaning,
      features: [
                      { icon: 'github-circle', title: theMeaning, content: `<span>Open source on <a href="https://github.com/buefy/buefy"> GitHub</a></span>` },
                      ...
                      { icon: 'arrange-bring-to-front', title: 'Lightweight', content: `<span>No other internal dependencies</span>` }
                  ]
    }
  }
}
